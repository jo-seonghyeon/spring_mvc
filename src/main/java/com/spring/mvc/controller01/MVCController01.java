...
public class MVCController01 {
    ...
    public String phone(PhoneDTO phone){
        System.out.println("파라미터로 전달받은 PhoneDTO 내부 : " + phone);
        return "";
    }
    ...
    public String member(@PathVariable String mName, @PathVariable String mNo) {
    ...
    // 데이터를 보낼 때는 당연히 폼이라는 양식을 제공하고, 거기에 대해서 적절한 처리를 하는
    // 처리 메서드를 만들게 됩니다.
    ...
    @GetMapping("/cat") // http://localhost:8080/cat 패턴 생성
    public String cat(String name, int age, String kind) {
        ...
        // chap01 폴더 내부의 catreq.jsp로 연결되도록 해주세요.
        return "/chap01/catreg";
    }

    // 아이디, 비밀번호를 각각 id, pw라는 name에 전달해주는 폼을 생성해주세요
    ...
    @RequestMapping("login") // 보낸 아이디와 비밀번호를 받아서 화면에 찍어주는 로직을 작성해주세요.
