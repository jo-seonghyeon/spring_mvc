...
public class MVCController03 {
    ...
    public String naver(){
        ...
        // 마치 a태그를 클릭하는것처럼 자동으로 페이지를 보낼 주소로 이동시킵니다.
    ...
    public String goFoodForm(Model model){
        ...
        model.addAttribute("test", "test"); // title이라는 이름으로 "음식주문..."문자열 적재
         return "redirect:/chap03/foodform"; // 목적 파일로 포워딩하지 않고 목적 메서드 주소로 리다이렉트
        // /WEB-INF/views/chap03/foodform.jsp 라는 목적파일로 포워딩
        //return "/chap03/foodform"; // 목적 파일인 foodform.jsp로 적재 데이터도 보내는 포워딩 방식

    }
    // 리다이렉트는 데이터 전달이 불가능한가요?
    // GET방식은 url에 쿼리스트링을 붙여서 데이터를 전송하기 때문에
    // 리다이렉트 타켓 URL에 쿼리스트링을 붙여 리다이렉트를 수행하면 전달 가능합니다.
    ...
    public String reSearch(String query){
        String resultPage = "redirect:http://search.naver.com/search.naver?query=" + query;
        return resultPage;
    }
